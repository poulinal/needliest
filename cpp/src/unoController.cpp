...
// Out-of-line implementations for UnoController declared in UnoController.h

UnoController::UnoController() {
    setup();
}

void UnoController::setup() {
    pinMode(CW, OUTPUT);
    pinMode(CCW, OUTPUT);
...
float UnoController::setAngle(double current, double set) { // measured in # of CW rotations from closed position
...
float UnoController::pneumaticReadingRaw() {
...
float UnoController::pneumaticReadingRotation() {
    // Read potentiometer on A5
    uint16_t adc = analogRead(A5);
    // Convert ADC reading to number of rotations (0..6.0)
    float rotations = ((float)adc / 1023.0f) * 6.0f;
